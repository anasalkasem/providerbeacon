...
The Groups section is a community directory for Telegram, WhatsApp and Discord, filtered by language and purpose. Signed-in visitors with verified emails can submit a group through My groups, see their review notes, edit or hide their own submissions, and report published links. Staff must review the destination before publication. Editing a published group returns it to review. A provider association needs a source page on the provider's own website and a staff check. A reviewed link is not a guarantee of offers or members. You cannot retrieve group listings, join groups or moderate them: guide visitors to Groups in the navigation and never invent a group, its activity or its availability.
