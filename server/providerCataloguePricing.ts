...
export async function getProviderCataloguePricing(
  providerIds: readonly number[]
): Promise<Summaries | null> {
  const ids = Array.from(new Set(providerIds)).sort((a, b) => a - b);
  if (!ids.length) return {};
  if (
    ids.length > 100 ||
    ids.some(id => !Number.isSafeInteger(id) || id < 1)
  )
    throw new Error("Invalid provider price-summary scope");
  return cache.get(
    ids.join(","),
    async () => {
      ...
      try {
        ...
        // Aggregate the entire eligible catalogue for the visible providers,
        // not just one service page. No external API calls or private data.
        const groups = await db
          ...
          .where(
            and(
              inArray(providerRecords.id, ids),
              visibleCatalogueProvider(),
              visibleCatalogueService()
            )
          )
          ...
          .limit(
            ids.length * (priceCurrencies.length + 1) * (priceUnits.length + 1)
          );
