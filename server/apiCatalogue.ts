...
export function confirmedSourcePricing() {
  return and(inArray(serviceRecords.sourceCurrency, priceCurrencies), inArray(serviceRecords.sourcePriceUnit, priceUnits),
    isNotNull(serviceRecords.sourcePricingConfirmedAt), isNotNull(serviceRecords.sourcePricingIdentity),
    isNotNull(serviceRecords.sourcePricingEvidenceUrl),
    or(sql`${serviceRecords.sourcePriceUnit} <> 'package'`, sql`char_length(trim(${serviceRecords.sourcePackageDescription})) >= 8`));
}
export const cataloguePriceUnit = () => sql<"per_1000" | "per_item" | "package" | null>`case when ${serviceRecords.reviewStatus} = 'pending' then case when ${confirmedSourcePricing()} then ${serviceRecords.sourcePriceUnit} else null end else ${serviceRecords.priceUnit} end`;
export const cataloguePriceAmount = () => sql<string>`case when ${serviceRecords.reviewStatus} = 'pending' then cast(${serviceRecords.sourceRate} as decimal(65,30)) else ${serviceRecords.priceAmount} end`;
...
export function connectedApiCatalogue() {
  return and(eq(providerRecords.apiCataloguePublished, true), sql`exists (
...
export function sourceCatalogueService() {
  return and(connectedApiCatalogue(),
    ...
    isNotNull(serviceRecords.externalId), isNotNull(serviceRecords.sourceUpdatedAt),
