...
The central sync-alert page also shows quarantined records from each connection's last completed import, with its completion time and the same on-demand, 25-row report. Starting or failing a newer import cannot clear that alert; a later completed snapshot without source issues clears it while retaining historical evidence. The lookup matches the connection's current provider and endpoint, uses the existing job index and selects compact metadata only. The list is capped at 50 connections and refreshes every 30 seconds. Failed, overdue and quarantined states can appear together on one connection, counted once. Disabled scheduling does not hide outstanding source issues. Both alert counts and issue details retain the existing server-side `integrations.read` permission.
...
Alert acceptance cases verify persistence during queued/importing retries and a later failure, clearing after a clean completed snapshot, retained historical reports, edited provider/endpoint boundaries, and a 52-connection fixture with a 50-item response cap and separate error/overdue reasons.
