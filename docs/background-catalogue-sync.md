...
The production connection returned more than 5,000 services. The previous import rejected the complete response and ran all database writes inside the administrator's HTTP request. Increasing that limit alone would extend request duration and hold provider locks across a large transaction.
...
- The complete response is validated before changing service records. Empty responses, duplicate IDs and invalid required values fail without applying catalogue changes. Normalization yields to the event loop every 50 rows.
- Validated rows are staged temporarily in MySQL. Every import transaction processes at most 100 services and commits its service changes, price history, audit entries and progress checkpoint together. It reads only matching metadata for that batch.
...
- Completed and failed jobs release the active-provider key. Temporary staging rows are deleted in batches of 1,000. Compact job metadata and the audit log remain available. Historical audit retention is a separate policy.
