...
Signed-in members use a dedicated dashboard at `/account/provider`. All members see the complete navigation, including members without a linked provider. Paid owners land on an overview with measured visits and contact clicks, current publication totals, review and renewal reminders, and the monthly offer allowance. Separate navigation opens analytics, VIP, groups, offers, plan/payment history and ownership requests. The `provider` and `tab` query parameters preserve the selected provider and section on reload and browser navigation. Payment returns keep their verification flow and initially open billing. The navigation collapses on mobile and follows the current language, including Arabic RTL.

Free members land on an explorable tools overview. Locked navigation buttons remain operable and open feature-specific previews, labelled as previews without invented metrics. Analytics previews make no analytics requests. Unlinked members and members with invalid ownership/email verification never mount saved-content queries. The next action distinguishes email verification, free ownership verification, activation, renewal, scheduled start and suspension; payment never verifies ownership. Suspended plans point to support rather than another payment. Complimentary VIP placement does not unlock the paid toolkit.

Expired verified owners can still read and withdraw saved VIP cards, groups and offers beneath the relevant preview, while publishing and editing remain locked. Empty disabled editors are omitted. The existing workspace refresh/payment-confirmation flow unlocks the current section after activation, and the live clock locks it again at expiry. These are presentation changes; server-side entitlement gates, pricing, publication review and payment processing remain authoritative and unchanged.
