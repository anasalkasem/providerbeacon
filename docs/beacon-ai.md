...
- Prices and totals come from exact decimal arithmetic on recorded pricing evidence. Model prose is instructed to leave numbers and provider names to the cards. Free-form model explanations remain probabilistic; production evaluation with a real API key is required, especially for Arabic intent and follow-up requests.
