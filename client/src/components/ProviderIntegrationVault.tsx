...
export default function ProviderIntegrationVault() {
  ...
  const sync = trpc.admin.integrations.syncNow.useMutation({ onSuccess: async data => { toast.success(`${text("importedServices")}: ${"importedCount" in data ? data.importedCount : 0} · ${text("pendingReview")}: ${"reviewCount" in data ? data.reviewCount : 0}`); await Promise.all([refresh(), utils.admin.services.list.invalidate(), utils.marketplace.snapshot.invalidate()]); }, onError: error => toast.error(error.message) });
