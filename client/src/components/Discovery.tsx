    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {guides.map(g => (
        <Link
          ...
          className="group flex flex-col rounded-2xl border border-border bg-card p-6 transition hover:-translate-y-1 hover:border-input shadow-none "
        >
          ...
          <h3 className="mt-6 text-xl font-extrabold leading-relaxed text-foreground">
    ...
    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
      {profiles.map(p => (
        ...
        >
          ...
          <h3 className="mt-5 text-lg font-extrabold text-foreground" dir="ltr">
            {p.name}
          </h3>
          ...
          <p className="mt-4 text-xs text-muted-foreground">{t.publicSource}</p>
          <Link
            href={`/directory/${p.slug}`}
            className="mt-4 flex items-center justify-between border-t border-border pt-4 text-sm font-bold text-foreground"
          >
            {t.viewProfile}
...
export function SourceLink({
  ...
  return (
    <a
      ...
      className="inline-flex items-center gap-2 text-sm font-bold text-foreground underline decoration-input underline-offset-4"
    >
      {children}
