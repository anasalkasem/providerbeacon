...
export default function OfferEvidence({ service, showTerms = false }: { service: Service; showTerms?: boolean }) {
  ...
  return <div className="mt-3 max-w-sm space-y-2 text-xs leading-5 text-slate-500">
    {service.catalogueListing === "api_source" && <p className="font-bold text-beacon-700">{ar ? "مستورد من اتصال API الفعلي" : "Imported through the connected API"}</p>}
    {showTerms && serviceTerms(locale, service) && <p>{serviceTerms(locale, service)}</p>}
    <a className="inline-block font-bold text-beacon-700 underline underline-offset-4" href={service.sourceUrl} target="_blank" rel="noopener noreferrer">{ar ? "راجع السعر لدى المزود" : "Check price at source"}</a>
