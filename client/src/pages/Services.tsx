...
export default function Services() {
  ...
  return <PublicLayout>
    ...
    <section className="container py-8"><div className="filter-bar"><label className="relative flex-1"><span className="sr-only">{t.searchCatalogue}</span><Search className="absolute start-4 top-1/2 size-4 -translate-y-1/2 text-slate-400"/><Input value={query} onChange={event => setQuery(event.target.value)} placeholder={t.searchServices} className="h-12 rounded-xl border-slate-200 ps-11"/></label>
      <Select value={platform} onValueChange={setPlatform}><SelectTrigger className="h-12 w-full rounded-xl border-slate-200 sm:w-[190px]"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="all">{t.allPlatforms}</SelectItem>{["Instagram","TikTok","YouTube","Facebook","Telegram"].map(item => <SelectItem value={item} key={item}>{item}</SelectItem>)}</SelectContent></Select>
      <Select value={sort} onValueChange={setSort}><SelectTrigger className="h-12 w-full rounded-xl border-slate-200 sm:w-[190px]"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="recommended">{t.recommended}</SelectItem><SelectItem value="price">{t.lowestPrice}</SelectItem><SelectItem value="retention">{t.bestRetention}</SelectItem></SelectContent></Select>
      ...
      <div className="mt-6 flex items-center justify-between gap-4"><p className="text-sm text-slate-500"><strong className="text-slate-900">{formatNumber(locale, filtered.length)}</strong> {t.servicesFound}</p><div className="flex items-center gap-2 text-xs text-slate-500"><ListFilter className="size-4"/>{t.updatedContinuously}</div></div>
      <div className="mt-5 grid gap-3 md:hidden">{filtered.map(service => { const provider = providerFor(service); const checked = selected.some(item => item.id === service.id); return <article key={service.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><ProviderAvatar provider={provider}/><div className="min-w-0"><p className="truncate text-xs font-semibold text-slate-500">{provider.name} · {t.score} <bdi>{provider.score}</bdi></p><h2 className="mt-1 text-sm font-extrabold text-slate-900">{localizeData(locale, service.name)}</h2></div></div><button onClick={() => toggle(service)} aria-label={`${checked ? t.removeFromComparison : t.addToComparison}: ${localizeData(locale, service.name)}`} className={`grid size-8 shrink-0 place-items-center rounded-lg border ${checked ? "border-cyan-500 bg-cyan-500 text-white" : "border-slate-200"}`}>{checked ? <Check className="size-4"/> : <span className="text-lg leading-none">+</span>}</button></div><div className="mt-4 grid grid-cols-3 gap-2 rounded-xl bg-slate-50 p-3"><div><p className="text-[9px] uppercase text-slate-400">{t.pricePerThousand}</p><bdi dir="ltr" className="mt-1 font-extrabold">${service.pricePerThousand.toFixed(2)}</bdi></div><div><p className="flex items-center gap-1 text-[9px] uppercase text-slate-400"><Clock3 className="size-3"/>{t.start}</p><bdi dir="ltr" className="mt-1 text-xs font-bold">{localizeDuration(locale, service.startTime)}</bdi></div><div><p className="flex items-center gap-1 text-[9px] uppercase text-slate-400"><ShieldCheck className="size-3"/>{t.refill}</p><p className="mt-1 text-xs font-bold">{localizeData(locale, service.refill)}</p></div></div></article>; })}</div>
      ...
      {filtered.length === 0 && <div className="grid place-items-center px-6 py-20 text-center"><Search className="size-8 text-slate-300"/><h2 className="mt-4 font-bold text-slate-900">{t.noMatches}</h2><p className="mt-2 text-sm text-slate-500">{t.noMatchesBody}</p></div>}
    </section>
