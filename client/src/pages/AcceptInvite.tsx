...
import { teamWords, teamError } from "@/i18n/team";
import { inviteTokenFromLocation } from "@shared/team";
...
export default function AcceptInvite() {
  ...
  const [token] = useState(() =>
    inviteTokenFromLocation(window.location.search, window.location.hash)
  );
  ...
  const accept = trpc.admin.acceptInvite.useMutation({
    onSuccess: async () => {
      await utils.admin.access.invalidate();
    },
  });
  const register = trpc.auth.registerInvite.useMutation({
    onSuccess: async () => {
      ...
      <main className="container grid min-h-[76vh] place-items-center py-16">
        <section className="w-full max-w-xl rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50">
          ...
          {invalid ? (
            ...
            <Loader2 className="mx-auto mt-7 size-6 animate-spin" />
          ) : user ? (
            accept.isSuccess ? (
              <Button asChild className="mt-7 w-full rounded-xl">
                <Link href="/admin">{w.open}</Link>
              </Button>
            ) : (
              <Button
            ...
            >
              ...
              <Button
                ...
                disabled={register.isPending || password !== confirmation}
              >
                {register.isPending && (
                  <Loader2 className="size-4 animate-spin" />
                )}
          ...
          {(accept.error || register.error) && (
            <p className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-700">
              {(
                ...
                ? teamError(
                    locale,
                    accept.error?.message ?? register.error?.message ?? ""
                  )
                : (accept.error?.message ?? register.error?.message)}
            </p>
          )}
