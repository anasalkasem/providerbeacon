...
export default function Providers() {
  ...
  const visible = useMemo(() => providers.filter(provider => `${provider.name} ${provider.location} ${provider.description} ${localizeData(locale, provider.location)} ${localizeData(locale, provider.description)}`.toLowerCase().includes(query.toLowerCase())), [locale, providers, query]);
  return <PublicLayout>
    <section className="border-b border-slate-200 bg-white"><div className="container grid gap-8 py-14 lg:grid-cols-[1fr_420px] lg:items-end"><div><div className="eyebrow light"><Sparkles className="size-4"/>{t.providersEyebrow}</div><h1 className="mt-5 text-4xl font-extrabold tracking-tight text-slate-950 sm:text-6xl">{t.providersTitle}</h1><p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">{t.providersBody}</p></div><label className="relative"><span className="sr-only">{t.searchProviders}</span><Search className="absolute start-4 top-1/2 size-5 -translate-y-1/2 text-slate-400"/><input value={query} onChange={event => setQuery(event.target.value)} className="h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 ps-12 pe-4 text-sm outline-none focus:border-blue-500 focus:bg-white" placeholder={t.searchProviders}/></label></div></section>
    <section className="container py-12"><div className="flex items-center justify-between"><div><p className="section-kicker">{t.evaluatedProfiles}</p><h2 className="mt-2 text-2xl font-extrabold text-slate-950">{formatNumber(locale, visible.length)} {t.providersFound}</h2></div><div className="hidden items-center gap-2 text-sm font-semibold text-emerald-700 sm:flex"><ShieldCheck className="size-4"/>{t.identityRanking}</div></div><div className="mt-7 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{visible.map(provider => <ProviderCard key={provider.id} provider={provider}/>)}</div></section>
