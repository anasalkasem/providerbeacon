...
export default function Login() {
  ...
  return <PublicLayout><main className="container grid min-h-[76vh] place-items-center py-16"><section className="w-full max-w-md overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-2xl shadow-slate-200/70"><div className="bg-[#071A35] p-7 text-white"><div className="grid size-12 place-items-center rounded-2xl bg-cyan-400/15 text-cyan-300">{mfaRequired ? <ShieldCheck/> : <LockKeyhole/>}</div><h1 className="mt-5 text-3xl font-extrabold">{text("signIn")}</h1><p className="mt-2 text-sm leading-6 text-slate-300">{mfaRequired ? text("mfaBody") : text("signInBody")}</p></div>
