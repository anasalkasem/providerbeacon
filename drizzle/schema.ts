...
export const providerRecords = mysqlTable("provider_records", {
  ...
  websitePreviewUrl: varchar("websitePreviewUrl", { length: 500 }),
...
export const serviceRecords = mysqlTable("service_records", {
  ...
  sourcePricingIdentity: varchar("sourcePricingIdentity", { length: 64 }),
  ...
  countryCode: varchar("countryCode", { length: 2 }),
  screeningStatus: mysqlEnum("screening_status", ["pending", "clear", "held", "review", "manual_clear"]).default("pending").notNull(),
  screeningRevision: int("screening_revision").default(0).notNull(),
  screeningReason: varchar("screening_reason", { length: 40 }).default("none").notNull(),
  screeningEvidence: varchar("screening_evidence", { length: 500 }),
  screeningCheckedAt: timestamp("screening_checked_at"),
  screeningNextAt: timestamp("screening_next_at"),
  screeningAttempts: int("screening_attempts").default(0).notNull(),
  screeningError: varchar("screening_error", { length: 40 }),
  screeningModel: varchar("screening_model", { length: 100 }),
  ...
  index("service_screening_status_idx").on(table.screeningStatus, table.id),uniqueIndex("service_provider_slug_unique").on(table.providerId, table.slug), index("service_marketplace_idx").on(table.status, table.platform, table.category),
...
export const siteAppearance = mysqlTable("site_appearance", {
  ...
  theme: varchar("theme", { length: 24 }).default("beacon").notNull(),
  revision: int("revision").default(1).notNull(),
